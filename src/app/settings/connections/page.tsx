...
import { ArrowLeft, Calendar, Loader2, X, AlertTriangle } from 'lucide-react'
...
function ConnectionsContent() {
  ...
  useEffect(() => {
    ...
    } else if (conflictEmail && pendingId) {
      // Show conflict modal
      setConflictModal({
        email: conflictEmail,
        pendingId: pendingId,
  ...
  // Handle conflict resolution - connect calendar only
  const handleConnectCalendarOnly = async () => {
    if (!conflictModal) return
    setIsHandlingConflict(true)
    try {
      const response = await fetch('/api/calendar/callback/resolve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          pending_id: conflictModal.pendingId,
          action: 'calendar_only',
        }),
      })
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        setToast({ message: data.error || 'Failed to connect calendar', variant: 'error' })
        setConflictModal(null)
      }
    } catch (error) {
      console.error('Failed to resolve conflict:', error)
      setToast({ message: 'Failed to connect calendar', variant: 'error' })
    } finally {
  ...
  // Handle conflict resolution - sign in with Google instead
  const handleSignInWithGoogle = () => {
    setConflictModal(null)
    signOut({ callbackUrl: '/login?provider=google' })
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Conflict Modal */}
      <Dialog open={!!conflictModal} onOpenChange={(open) => !open && handleCancelConflict()}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              The Google account <span className="font-medium text-foreground">{conflictModal?.email}</span> is already registered as a user in Time Logger.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 py-4">
            ...
            >
              ...
              Sign in with this Google account instead
            </Button>

            <Button
              className="w-full justify-start"
              onClick={handleConnectCalendarOnly}
              disabled={isHandlingConflict}
            >
              {isHandlingConflict ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ...
              Just connect for calendar only
            </Button>
          </div>
